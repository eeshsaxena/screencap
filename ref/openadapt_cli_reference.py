...
@click.option("--window-data/--no-window-data", default=False, help="Capture window/accessibility data (macOS)")
def capture_start(name: str, video: bool, audio: bool, window_data: bool):
    ...
    try:
        ...
        with Recorder(
            ...
            capture_video=video,
            capture_audio=audio,
            capture_window_data=window_data,
        ) as recorder:
...
@main.group()
def privacy():
    """Detect and scrub PII/PHI from captures.

    \b
    Examples:
        openadapt privacy detect my-task
        openadapt privacy scrub my-task
        openadapt privacy scrub-image screenshot.png
    """
    pass


@privacy.command("detect")
@click.argument("capture_dir")
def privacy_detect(capture_dir: str):
    """Detect PII/PHI in a capture (text events only)."""
    try:
        from pathlib import Path
        ...
        if not capture_path.exists():
            click.echo(f"Error: Capture not found: {capture_dir}", err=True)
            sys.exit(1)
        ...
        for event in events:
            ...
            for key, value in event_data.items():
                if isinstance(value, str) and len(value) > 1:
                    ...
                    if scrubbed != value:
                        detections += 1
                        click.echo(f"  [{event.type}] {key}: {value!r} -> {scrubbed!r}")

        storage.close()
        click.echo(f"\nDetected PII/PHI in {detections} field(s)")

    except ImportError as e:
        click.echo(f"Error: Missing dependency: {e}", err=True)
        click.echo("Install with: pip install openadapt-privacy", err=True)
        sys.exit(1)


@privacy.command("scrub")
@click.argument("capture_dir")
@click.option("--video/--no-video", default=True, help="Also scrub video frames")
def privacy_scrub(capture_dir: str, video: bool):
    """Scrub PII/PHI from a capture's text and optionally video."""
    try:
        from pathlib import Path
        ...
        scrubber = ScrubProvider.get_scrubber(ScrubProvider.PRESIDIO)
        click.echo(f"Scrubbing PII/PHI in: {capture_dir}")

        # Scrub video if requested
        if video:
            video_path = capture_path / "video.mp4"
            if video_path.exists():
                click.echo("Scrubbing video (this may take a while)...")
                try:
                    output = scrubber.scrub_mp4(str(video_path))
                    click.echo(f"  Scrubbed video: {output}")
                except NotImplementedError:
                    click.echo("  Video scrubbing not supported by this provider")
            else:
                click.echo("  No video file found, skipping")
...
@privacy.command("scrub-image")
@click.argument("image_path")
@click.option("--output", "-o", help="Output path (default: <name>_scrubbed.<ext>)")
def privacy_scrub_image(image_path: str, output: Optional[str]):
    """Scrub PII/PHI from a single image."""
    try:
        from pathlib import Path
        ...
        click.echo(f"Scrubbing: {image_path}")
        scrubbed = scrubber.scrub_image(image)

        out_path = output or str(img_path.stem) + "_scrubbed" + img_path.suffix
...
def ground():
    ...
    Examples:
        ...
        openadapt ground locate screenshot.png --query "Submit button"
    """
    pass


@ground.command("detect")
@click.argument("image_path")
@click.option("--parser", "-p", default="omniparser", help="Parser to use")
@click.option("--output", "-o", help="Save annotated image to path")
def ground_detect(image_path: str, parser: str, output: Optional[str]):
    ...
    try:
        ...
        click.echo(f"Detecting UI elements in: {image_path}")
        ...
        for i, elem in enumerate(result.elements):
            label = getattr(elem, "label", getattr(elem, "text", ""))
            bounds = getattr(elem, "bounds", None)
            click.echo(f"  [{i}] {label}  bounds={bounds}")

        if output:
            annotated = result.annotate(image)
            annotated.save(output)
            click.echo(f"\nAnnotated image saved: {output}")

    except ImportError as e:
        click.echo(f"Error: Missing dependency: {e}", err=True)
        click.echo("Install with: pip install openadapt-grounding", err=True)
        sys.exit(1)
    except Exception as e:
        click.echo(f"Error: {e}", err=True)
        sys.exit(1)
...
@main.group()
def retrieval():
    """Index and search demonstration recordings.

    \b
    Examples:
        openadapt retrieval index --captures task-1 task-2
        openadapt retrieval search --text "click submit"
    """
    pass


@retrieval.command("index")
@click.option(
    "--captures", "-c", multiple=True, required=True, help="Capture dirs to index"
)
@click.option("--embedder", "-e", default="clip", help="Embedder model (clip, qwen3vl)")
@click.option("--output", "-o", default="demo_index", help="Index output directory")
def retrieval_index(captures: tuple, embedder: str, output: str):
    """Index captures for semantic retrieval."""
    try:
        from openadapt_retrieval import MultimodalDemoRetriever, get_embedder

        click.echo(f"Indexing {len(captures)} capture(s) with {embedder}...")
        emb = get_embedder(embedder)
        retriever = MultimodalDemoRetriever(embedder=emb, index_dir=output)

        for cap_dir in captures:
            click.echo(f"  Indexing: {cap_dir}")
            retriever.index_capture(cap_dir)
        ...
        click.echo(f"Error: Missing dependency: {e}", err=True)
        click.echo("Install with: pip install openadapt-retrieval", err=True)
        sys.exit(1)
    except Exception as e:
        click.echo(f"Error: {e}", err=True)
        sys.exit(1)
...
@retrieval.command("search")
@click.option("--text", "-t", help="Text query")
@click.option("--image", "-i", help="Image query (screenshot path)")
@click.option("--index", default="demo_index", help="Index directory")
@click.option("--top-k", "-k", default=5, help="Number of results")
def retrieval_search(
    text: Optional[str], image: Optional[str], index: str, top_k: int
):
    """Search indexed demos by text or image similarity."""
    try:
        from openadapt_retrieval import MultimodalDemoRetriever, get_embedder

        if not text and not image:
            click.echo("Error: Provide --text or --image", err=True)
            sys.exit(1)

        emb = get_embedder("clip")
        retriever = MultimodalDemoRetriever(embedder=emb, index_dir=index)
        retriever.load()
        ...
        results = retriever.search(text=text, image=query_image, top_k=top_k)

        click.echo(f"Top {len(results)} results:")
        for i, r in enumerate(results):
            score = getattr(r, "score", getattr(r, "distance", "?"))
            source = getattr(r, "capture_dir", getattr(r, "source", "?"))
            click.echo(f"  [{i+1}] score={score:.4f}  source={source}")
