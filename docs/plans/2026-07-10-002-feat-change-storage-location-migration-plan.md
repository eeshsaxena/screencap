...
- **R5** — Preflight validation rejects unsafe targets before the move: not writable, on a different volume (R3), nested inside (or a parent of) the source, non-empty / already-existing-with-data, insufficient free space, an active `SCREENCAP_RECORDINGS_DIR` env override that would mask a config change, or **a cloud-synced folder** (iCloud Drive / Dropbox / OneDrive / Google Drive) that would silently sync the local-only library off-device.
...
- **R9** — The new recordings root and moved files carry restrictive permissions (`0o700` root, `0o600` files) matching the `models/` / `content_index.db` posture, so a move outside `~` does not expose recordings to other local users.
