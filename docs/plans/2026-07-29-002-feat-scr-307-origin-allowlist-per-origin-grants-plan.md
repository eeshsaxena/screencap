...
- **OQ4. Is the broad-grant probe actually functional?** `hasLiveBroadGrant` asks `permissions.contains` about `https://broad-grant-probe.invalid/*`, assuming Chrome answers `true` when a live all-sites grant subsumes it. **Unverified.** If Chrome's match-pattern parser instead rejects a `.invalid` host, `contains` throws, the guard catches it and returns `false`, and the broad-grant warning silently never fires.

  The failure is quiet rather than dangerous: that path degrades to the pre-fix behavior for one of the two triggers, while the `getAll()`-throws trigger — the one needing no Chrome-behavior assumption — is fixed unconditionally. Ship-blocking only if the all-sites grant path turns out to be reachable in practice (see OQ2). **Resolve** by loading the extension in a browser and evaluating `chrome.permissions.contains({origins:["https://broad-grant-probe.invalid/*"]})` with and without an all-sites grant; if the probe is unworkable, the honest fallback is to warn on the envelope's mere presence in `getAll()` and accept a false positive when the envelope is only declared.

- **OQ5. How will browser-level tests run at all?** An automated attempt to verify OQ1/OQ2/OQ4 against a real browser failed at the first step: **Chrome 150 and Brave 149 both ignore `--load-extension` entirely** — the switch was restricted around Chrome M137, and neither `--disable-features=DisableLoadExtensionCommandLineSwitch` nor `--enable-unsafe-extension-debugging` restores it on these stable builds. Nothing registers, headless or headful. Every Chrome-behavior assumption in this plan therefore rests on documentation and community reports, not observation. This blocks `parent U3`-`parent U5` harder than it blocks U2, since those units' verification statements are all browser-level. **Resolve** by pinning a browser that still permits unpacked loads — Chrome for Testing via `@puppeteer/browsers`, or Playwright's bundled Chromium — as a dev dependency before the capture units land.

None of the five blocks an implementation unit. OQ1 is settled inside U1; OQ2 and OQ4 need a browser and are bounded by the fail-quiet argument above; OQ3 belongs to a later unit; OQ5 is infrastructure the capture units will force.
