...
- **OQ4. RESOLVED — the broad-grant probe works.** Measured against Chrome for Testing 151 with the extension loaded unpacked: `contains({origins:["https://broad-grant-probe.invalid/*"]})` is `false` with nothing granted and `true` under a live `https://*/*` grant. Chrome accepts a `.invalid` host in a match pattern rather than rejecting it, so the probe is functional rather than dead code.

  The same run resolved the ambiguity the probe was built for: with the envelope declared *optional* and nothing granted, `getAll().origins` is `[]`. The declared-but-ungranted envelope is **not** reported, so presence in `getAll()` already implies granted. The probe is kept as cheap confirmation against those semantics changing, not as a necessity — and it runs only on a popup read, never on the capture path.

  It also confirmed the P1 was genuine and not theoretical: under a live all-sites grant, `contains` answers `true` for an arbitrary unrelated host, which is exactly the fail-open the state reporting now prevents.

- **OQ5. A browser harness exists and works; my earlier claim here was wrong.** This entry previously recorded that Chrome 150 and Brave 149 "ignore `--load-extension` entirely". **That was a misdiagnosis.** The switch works. The bug was in the detection: the harness identified the extension by looking for its path in the profile's `Preferences` file, which is never written for a command-line-loaded extension — and on the first pass it attached to whichever `service_worker` target appeared first, which is one of Chrome's own component extensions (Hangouts, Gemini in Chrome), answering every question about the wrong extension while looking like a success.

  Two rules for the harness `parent U3`-`parent U5` will need: **identify the extension by evaluating `chrome.runtime.getManifest().name`, never by target order or profile files**, and **grant host permissions by writing `host_permissions` into a throwaway copy of the manifest** (auto-granted for an unpacked load) rather than trying to drive `permissions.request()`. The harness prototype lives outside the repo; landing it as a dev dependency is follow-up work, not U2's.

**Verified in the same run, and each already assumed by this plan:** an exact-host grant does **not** cover its subdomains (`KTD6` holds — `https://example.com/*` granted, `contains` on `https://app.example.com/*` is `false`); ports are ignored in matching, so a host grant covers every port on it (`OQ1`'s drop-the-port decision is right, and the popup's "this allows every port" notice is accurate); and a subdomain wildcard covers the bare parent too (`https://*.example.com/*` matches `example.com`), which is worth knowing but changes nothing here.

**Still open:** `OQ2` (what `chrome://extensions` shows after a `remove()`) and the interactive `permissions.request()` flow. Both need a human — `request()` genuinely requires a real user gesture, and CDP's synthetic one is refused with the exact error `KTD5` predicts, which independently confirms why the popup must call it directly. `OQ3` belongs to a later unit.
