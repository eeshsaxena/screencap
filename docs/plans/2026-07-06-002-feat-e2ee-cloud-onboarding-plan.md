...
- **KTD-5 — Signer unchanged; uploads become opaque; server-side *processing* breaks (default-on prerequisite).** The signing Cloud Function only signs URLs and never reads bytes, so ciphertext uploads transparently. Encrypted objects use `application/octet-stream` so nothing infers plaintext structure — and because the signed-URL request advertises a per-file `content_type` that a V4 signature may bind, encrypted files must send `octet-stream` in **both** the signed-URL request and the PUT header (a mismatch is a 403). **Correction (found in code review):** the *signer* is not the only server-side consumer. A separate deployed Eventarc handler, `scripts/process-recording/main.py`, `json.loads` the uploaded `recording_complete.json` sentinel (its sole live trigger, since `recording.db` is never uploaded) and parses the per-chunk manifests to drive session stitching, the website index, and Gemini enrichment. Under E2EE those artifacts are ciphertext, so that pipeline silently no-ops for every encrypted recording. This is harmless while the flag is default-off (no data loss; the app still downloads and decrypts), but it is a **hard prerequisite to resolve before turning the flag on** — tracked in SCR-238. The website's direct-from-GCS player breaks the same way (deferred; the macOS app is the viewer via download+decrypt).
...
- **Test scenarios:**
  ...
  - The engine reads the key from the delivered `0600` file (not the Keychain); the file is created with `O_NOFOLLOW` and a pre-planted symlink at its path is refused, not followed.
...
| Flag + key delivery | `PYTHONPATH=src pytest` on the config/auth/engine key-delivery tests (flag default-off, foreground key creation, `0600`/`O_NOFOLLOW` staging + unlink-on-exit) | U4 |
