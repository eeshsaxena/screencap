...
- **KTD1 — All cause resolution happens daemon-side in the day reader.** `src/screencap/day_segments.py` owns artifact reads, the purge join, and can cheaply peek at the previous day's last recording (a Swift-side resolver cannot see yesterday's dying recording, read stop artifacts, or read disable logs). The strip stays a dumb renderer. The read-only join also respects the package DAG: `enforcement` stays read-surface-free, `backfill` never imports `daemon`, and `day_segments` already imports `backfill.skip_intervals` (pinned by `tests/test_package_boundary_call_graph.py`).
