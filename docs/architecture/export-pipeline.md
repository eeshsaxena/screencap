...
## `build_cloud_window_filter` — the only sanctioned cloud constructor

`build_cloud_window_filter(cloud_bound: bool, privacy_mode, capture_dir) -> Callable | None` lives in `src/screencap/privacy/filter.py`. It is the single entry point cloud-bound callers MUST use.
...
- **Factory call-graph contract.** `build_cloud_window_filter` is the only sanctioned constructor for cloud-bound window filters. `build_privacy_filter` is module-private in spirit (called only by the factory and by direct CLI / test paths that explicitly opt out of cloud mode). Enforced by the AST walker in `tests/test_privacy_filter_call_graph.py`. A future cloud-bound caller that imports `build_privacy_filter` directly, or passes `window_filter=None` literally to `unified_export_events`, fails the build.
