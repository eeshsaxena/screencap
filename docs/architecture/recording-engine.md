...
- **Heavy imports deferred.** `engine/__init__.py` imports `Recorder` inside a try/except so the package is import-safe in headless environments. CLI commands import their dependencies inside the function body.
