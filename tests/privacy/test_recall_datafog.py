"""End-to-end recall/precision benchmark for the DataFog NER pipeline.

Mirror of test_recall.py but using DataFogPiiDetector instead of Presidio.
...
def pipeline() -> DetectionPipeline:
    """Full pipeline with DataFog NER instead of Presidio."""
...
class TestDataFogRecallBenchmark:
    def test_recall_above_threshold(self, pipeline: DetectionPipeline):
        ...
        print(f"\n[DataFog] Recall: {total_found}/{total_expected} = {recall:.1%}")
        if misses:
...
class TestDataFogPrecisionBenchmark:
    def test_precision_above_threshold(self, pipeline: DetectionPipeline):
        ...
        if false_positives:
            print(f"\n[DataFog] False positives ({len(false_positives)}):")
        ...
        print(f"\n[DataFog] Precision: {total_tp}/{all_detections} = {precision:.1%}")
...
class TestDataFogAnonymizerEndToEnd:
