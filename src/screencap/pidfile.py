...
def _is_screencap_process(pid: int) -> bool:
    """Check if a PID is actually a screencap/openadapt_capture process."""
    try:
        proc = psutil.Process(pid)
        cmdline = " ".join(proc.cmdline()).lower()
        return "openadapt_capture" in cmdline or "screencap" in cmdline
...
def find_orphaned_processes() -> list[dict[str, int | str]]:
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            if (
                proc.pid != os.getpid()
                and ("openadapt_capture" in cmdline or "screencap" in cmdline)
