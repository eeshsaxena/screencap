"""Context classification for privacy v3 (shared core).

Pure, DB-free classification of app/window context for policy decisions.
Constructed on BOTH the capture-time enforcement path
(``screencap.enforcement.recorder_enforcement``) and the post-hoc scrub path
(``scrubber`` / ``video_mask`` / ``chunk_processor``), so it lives in the
shared ``screencap.privacy`` leaf.

Owns:
- bundle-ID → ContextClass mapping
- domain evidence extraction (``domain_from_url``)
- title heuristic enrichment
- the deterministic ``DefaultContextClassifier`` state machine

This module must NOT import ``screencap.recording_db`` or
``screencap.redaction``; the scrub-time DB-geometry readers that carry that
dependency live in ``screencap.redaction.geometry``.
"""
