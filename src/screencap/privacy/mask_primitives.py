"""Shared pixel/region/geometry primitives for structural masking.

These low-level primitives generate mask geometry from window state and
render mask fills onto Pillow images. They are consumed by BOTH the
capture-time path (``RecorderPrivacyFilter.mask_frame`` in
``screencap.enforcement``) and the post-hoc scrub path
(``screencap.redaction.masking``).

Because the capture path must never depend on ``screencap.redaction``,
these primitives live in the shared ``screencap.privacy`` core leaf. The
policy-driven scrub-time orchestration (``MaskStrategy`` /
``get_mask_strategy`` / ``mask_screenshot``) lives in
``screencap.redaction.masking`` and imports these primitives one-way.

Owns:
- mask region type (``MaskRegion``)
- mask geometry generation (full-window, pane, per-window from geometry)
