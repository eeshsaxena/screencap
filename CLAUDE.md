...
### Encrypted-at-rest container (SCR-236)

The local recording data plane (recordings tree + content index + backfill ledger) lives inside an app-managed **encrypted sparse bundle** (`~/.screencap/store.sparsebundle`, AES-256/APFS), mounted transparently at `~/.screencap/recordings` while the store is in use so **every existing reader works unchanged** (KTD-2: the mountpoint *is* the recordings dir, no symlinks). `src/screencap/container.py` is the single `hdiutil` wrapper (create/attach/detach/compact/status) plus the Keychain key lifecycle (service `com.screencap.container`, read-only-by-default so a lost key never orphans a store), the typed failure taxonomy (retryable→`EX_TEMPFAIL` / operator→exit 1), and `ensure_store_mounted()` — the **one mount owner**, serialized by `~/.screencap/run/mount.lock` (`fcntl.flock`), that the daemon (`serve()` mounts before `bind_unix_socket`, KTD-3) and the CLI funnel (`config.get_recordings_dir`, KTD-3) both call. The mount **outlives the daemon** (KTD-4): the Swift app and MCP agents read frames directly off disk with no daemon in the byte path, so unmount happens only on `serve --uninstall`, `store lock`, or logout. `config.container_active()` (flag `container_enabled`, **default on**; bypassed by `SCREENCAP_RECORDINGS_DIR` or a non-default `recordings_dir`) is the single "is the container live" predicate the daemon, the disk guard (which measures the **host** volume backing the bundle, not the mounted volume's virtual free space, KTD-13), and the CLI all gate on. The `screencap store` group (`init`/`lock`/`unlock`/`compact`) and `serve --install` (foreground key+bundle creation) are the lifecycle entry points; there is **no migration and no reverse migration in v1**, and key loss is unrecoverable. FileVault is a warn-only signal on `/v0/daemon.info`. See `SECURITY.md` for the at-rest trust boundary (what the container protects vs. the live same-EUID / Keychain / run-dir / QuickLook residuals).
