...
- `recorder.py` — wraps `screencap.engine.Recorder` context manager. Signal handlers installed via `RecordingPolicies.signal` BEFORE `Recorder.__enter__()` to cover the entire setup window — enforced by `tests/test_signal_during_setup.py` (Tier-3 subprocess SIGINT-into-the-gap test). The standalone CLI passes `ThreeTapSigint`; session workers pass `NoopSignalPolicy` because `SessionController` owns Ctrl+C. SIGINT: first = graceful stop, second = force quit (kills children + `os._exit`), third = immediate `os._exit`. SIGTERM: graceful stop (used by `screencap stop`). Handlers guard against `recorder=None` (signal during setup) and fall back to `multiprocessing.active_children()` when `_child_pids` is empty. The recommended way to stop a recording programmatically is `screencap stop` (sends SIGTERM, 30s timeout, fallback to force-kill). Ctrl+C is a convenience shortcut for foreground terminal use only.
